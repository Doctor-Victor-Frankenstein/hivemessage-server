...
let _options = {
	logging_level: 3,
	rpc_error_limit: 5,
	rpc_nodes: ["https://api.openhive.network", "https://anyx.io", "https://api.hive.blog"],
...
async function broadcast(method_name, params, key) {
	return new Promise(async (resolve, reject) => {
		for(let i = 0; i < clients.length; i++) {
			...
			try {
				let result = await tryHiveBroadcast(clients[i], method_name, params, key);
...
async function tryHiveBroadcast(client, method_name, params, key) {
	return new Promise(async (resolve, reject) => {
...
async function processBlock(block_num, errors = 0) {
	...
	if(!block || !block.transactions) {
		...
		if(errors >= 8) {  return; }
		return processBlock(block_num, errors + 1);
	}
...
function saveState(last_block, block_time) {
  // Save the last block read to disk
  fs.writeFile('state.json', JSON.stringify({ last_block, block_time }), function (err) {
    if (err)
      utils.log(err);
  });
}
